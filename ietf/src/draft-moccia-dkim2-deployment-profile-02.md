...
docname: draft-moccia-dkim2-deployment-profile-03
category: info
ipr: trust200902
...
normative:
  ...
  I-D.ietf-dkim-dkim2-spec:
    ...
    date: 2026-03
    seriesinfo:
      Internet-Draft: draft-ietf-dkim-dkim2-spec-00
...
The per-recipient header approach used by DKIM2-core for envelope binding is the only architecture that achieves recipient-level attribution via a standard filter interface without MTA core modifications. Alternative approaches that bind recipient identity at the per-transaction level - such as DARA as deployed by Google Groups - require the MTA to split multi-recipient transactions into individual deliveries before signing. This splitting must occur before the signing component runs, making it a prerequisite of the signing process rather than an independent operation. A milter or equivalent filter cannot instruct the MTA to perform this splitting - it can only observe the recipients the MTA presents. This means that DARA-style envelope binding requires control over MTA delivery logic, which is not available through any standard filter interface. The base specification {{I-D.ietf-dkim-dkim2-spec}} encodes rt= values as base64 without JSON in its current revision, which reduces parsing overhead but retains opacity - the values are not directly human-readable without decoding. DKIM2-core's plaintext tag-value format for envelope binding fields remains directly inspectable without tooling. DKIM2-core achieves equivalent cryptographic binding of each recipient to the signed message via the standard milter envrcpt callback, listing all transaction recipients in DKIM2-Sig-rt fields. No MTA modification is required, making the mechanism deployable by any operator regardless of infrastructure.
...
- fr= (fragment index): MUST NOT exceed 20 per declaration. This supports header field values up to approximately 18KB, which is orders of magnitude beyond any header value observed in production deployments.
...
All DKIM2-core declarations - envelope binding, modification attribution, chain of custody - are in cleartext tag-value format, directly inspectable in mail logs without specialized tooling. This transparency supports real-time threat detection and incident response in ways that base64-encoded opaque blobs do not.
